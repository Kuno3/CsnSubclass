...
struct FixedThetaPostSampler
    ...
    function FixedThetaPostSampler(
        y,
        W,
        feature;
        nu2_beta=100.0,
...
function lp_s(sampler::FixedThetaPostSampler, s, u, rho, delta, omega2, eta)
    b = sqrt(2 / pi)
...
function s_sampler!(s, sampler::FixedThetaPostSampler, u, beta, sigma2, rho, delta, omega2, eta)
    b = sqrt(2 / pi)
    ...
    cov_s_pred_1 = Symmetric((1-delta^2) * gamma^2 * Omega_s * Omega_s)
    ...
    mean_s_pred[1, :] = gamma * delta * Omega_s * (u[1, :] .- b)
    ...
    for t in 2:sampler.T
        ...
        mean_s_pred[t, :] = rho * mean_s_filt[t-1, :] + gamma * delta * Omega_s * (u[t, :] .- b)
...
function u_sampler!(u, sampler::FixedThetaPostSampler, s, rho, delta, omega2, eta)
    b = sqrt(2 / pi)
    gamma = 1 / sqrt(1-b^2*delta^2)
    inv_Omega_s = (1 / sqrt(omega2)) * sampler.eigen_vector * Diagonal(sqrt.(eta * sampler.eigen_values_raw .+ (1-eta))) * sampler.eigen_vector'
...
function beta_sampler(sampler::FixedThetaPostSampler, s, sigma2)
    s_flatten = s'[:]
...
function sigma2_sampler(sampler::FixedThetaPostSampler, s, beta)
    s_flatten = s'[:]
...
function rho_sampler(sampler::FixedThetaPostSampler, s, u, delta, omega2, eta)
    b = sqrt(2 / pi)
    gamma = 1 / sqrt(1-b^2*delta^2)
    Omega_s = sqrt(omega2) * sampler.eigen_vector * Diagonal(1.0 ./ sqrt.(eta * sampler.eigen_values_raw .+ (1-eta))) * sampler.eigen_vector'
    inv_Omega_s = (1 / sqrt(omega2)) * sampler.eigen_vector * Diagonal(sqrt.(eta * sampler.eigen_values_raw .+ (1-eta))) * sampler.eigen_vector'
    ...
    mu_rho = 0
    for t in 2:sampler.T
        mu_s = gamma * delta * Omega_s * (u[t, :] - b * ones(sampler.K))
        s_scaled = (1 / (gamma * sqrt(1-delta^2))) * inv_Omega_s * s[t-1, :]
        prec_rho += s_scaled' * s_scaled
        mu_rho += (1 / (gamma * sqrt(1-delta^2))) * (s[t, :] - mu_s)' * inv_Omega_s * s_scaled
    end
    var_rho = 1 / prec_rho
    mu_rho /= prec_rho
...
function delta_sampler(sampler::FixedThetaPostSampler, s, u, rho, delta, omega2, eta, mu_delta, nu2_delta, delta_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
    ...
    lp_new = lp_s(sampler, s, u, rho, delta_prop, omega2, eta)
...
function omega2_sampler(sampler::FixedThetaPostSampler, s, u, rho, delta, omega2, eta, omega2_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
...
function eta_sampler(sampler::FixedThetaPostSampler, s, u, rho, delta, omega2, eta, eta_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
...
function mu_and_nu2_delta_sampler(sampler::FixedThetaPostSampler, delta)
    kappa0 = 0.01
...
function log_posterior(sampler::FixedThetaPostSampler, s, u, beta, sigma2, rho, delta, omega2, eta, mu_delta, nu2_delta, lp_old::Union{Float64, Nothing}=nothing)
    s_flatten = s'[:]
...
function sampling(
    ...
    @showprogress for i in 1:(burn_in + num_sample)
        for _ in 1:thinning
            ...
            delta, lp = delta_sampler(sampler, s, u, rho, delta, omega2, eta, mu_delta, nu2_delta, delta_prop_scale)
            omega2, lp = omega2_sampler(sampler, s, u, rho, delta, omega2, eta, omega2_prop_scale, lp)
