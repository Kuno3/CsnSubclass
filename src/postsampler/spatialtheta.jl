...
struct SpatialThetaPostSampler
    y::Array{Float64,2}
    W::Array{Float64,2}
    ...
    function SpatialThetaPostSampler(
        y,
        W,
        feature;
        nu2_beta=100.0,
...
function lp_s(sampler::SpatialThetaPostSampler, s, u, rho, delta, omega2, eta)
    b = sqrt(2 / pi)
    ...
    lp_s += sum(logpdf.(
        Normal.(
            gamma .* delta .* (u[1, :] - b * ones(sampler.K)),
    ...
    for t in 2:sampler.T
        ...
        lp_s += sum(logpdf.(
            Normal.(
                gamma .* delta .* (u[t, :] - b * ones(sampler.K)),
                gamma .* sqrt.(1 .- delta.^2)
            ),
            tilde_s
        ))
...
function s_sampler!(s, sampler::SpatialThetaPostSampler, u, beta, sigma2, rho, delta, omega2, eta)
    b = sqrt(2 / pi)
    Delta = Diagonal(delta)
    Gamma = Diagonal(1 ./ sqrt.(1 .- b^2 * delta.^2))
    ...
    cov_s_pred_1 = Symmetric(Omega_s * Gamma * (I - Delta * Delta) * Gamma * Omega_s)
    cov_s_pred[1] = cov_s_pred_1
    mean_s_pred[1, :] = Omega_s * Gamma * Delta * (u[1, :] .- b)
    ...
    for t in 2:sampler.T
        cov_s_pred[t] = Symmetric(rho^2 * cov_s_filt[t - 1] +cov_s_pred_1)
        mean_s_pred[t, :] = rho * mean_s_filt[t-1, :] + Omega_s * Gamma * Delta * (u[t, :] .- b)
...
function u_sampler!(u, sampler::SpatialThetaPostSampler, s, rho, delta, omega2, eta)
    b = sqrt(2 / pi)
    gamma = 1 ./ sqrt.(1 .- b^2 * delta.^2)
    inv_Omega_s = (1 / sqrt(omega2)) * sampler.eigen_vector * Diagonal(sqrt.(eta * sampler.eigen_values_raw .+ (1-eta))) * sampler.eigen_vector'

    tilde_s = inv_Omega_s * s[1, :]
    conditional_mu_u = delta ./ gamma .* tilde_s .+ b * delta.^2
    u[1, :] = conditional_mu_u + rand.(Truncated.(Normal.(0, sqrt.(1 .- delta.^2)), -conditional_mu_u, Inf))
    for t in 2:sampler.T
        tilde_s = inv_Omega_s * (s[t, :] - rho * s[t-1, :])
        conditional_mu_u = delta ./ gamma .* tilde_s .+ b * delta.^2
        u[t, :] = conditional_mu_u + rand.(Truncated.(Normal.(0, sqrt.(1 .- delta.^2)), -conditional_mu_u, Inf))
    end
end

function beta_sampler(sampler::SpatialThetaPostSampler, s, sigma2)
    s_flatten = s'[:]
...
function sigma2_sampler(sampler::SpatialThetaPostSampler, s, beta)
    s_flatten = s'[:]
...
function rho_sampler(sampler::SpatialThetaPostSampler, s, u, delta, omega2, eta)
    b = sqrt(2 / pi)
    gamma = 1 ./ sqrt.(1 .- b^2 * delta.^2)
    Delta = Diagonal(delta)
    Gamma = Diagonal(gamma)
    Omega_s = sqrt(omega2) * sampler.eigen_vector * Diagonal(1.0 ./ sqrt.(eta * sampler.eigen_values_raw .+ (1-eta))) * sampler.eigen_vector'
    inv_Omega_s = (1 / sqrt(omega2)) * sampler.eigen_vector * Diagonal(sqrt.(eta * sampler.eigen_values_raw .+ (1-eta))) * sampler.eigen_vector'
    ...
    mu_rho = 0
    for t in 2:sampler.T
        mu_s = Omega_s * Gamma * Delta * (u[t, :] .- b)
        s_scaled = Diagonal(1 ./ (gamma .* sqrt.(1 .- delta.^2))) * inv_Omega_s * s[t-1, :]
        prec_rho += s_scaled' * s_scaled
        mu_rho += (s[t, :] - mu_s)' * inv_Omega_s * Diagonal(1 ./ (gamma .* sqrt.(1 .- delta.^2))) * s_scaled
    end
    var_rho = 1 / prec_rho
    mu_rho /= prec_rho
...
function delta_sampler!(delta, sampler::SpatialThetaPostSampler, s, u, rho, omega2, eta, mu_delta, nu2_delta, delta_prop_scale)
    b = sqrt(2 / pi)
    inv_Omega_s = (1 / sqrt(omega2)) * sampler.eigen_vector * Diagonal(sqrt.(eta * sampler.eigen_values_raw .+ (1-eta))) * sampler.eigen_vector'
    dst_prior = Normal(mu_delta, sqrt(nu2_delta))

    tilde_s = zeros(sampler.T, sampler.K)
    tilde_s[1, :] = inv_Omega_s * s[1, :]
    for t in 2:sampler.T
        tilde_s[t, :] = inv_Omega_s * (s[t, :] - rho * s[t-1, :])
    end

    for k in 1:sampler.K
        delta_curr = delta[k]
        ...
        threshold = log(rand())
        delta[k] = log_accept_ratio > threshold ? delta_prop : delta_curr
    end
end

function omega2_sampler(sampler::SpatialThetaPostSampler, s, u, rho, delta, omega2, eta, omega2_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
...
function eta_sampler(sampler::SpatialThetaPostSampler, s, u, rho, delta, omega2, eta, eta_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
...
function mu_and_nu2_delta_sampler(sampler::SpatialThetaPostSampler, delta)
    kappa0 = 0.01
    lambda = delta ./ sqrt.(1 .- delta.^2)
    a_n = 1 + 0.5 * sampler.K
    b_n = 0.01 + 0.5 * sum((lambda .- mean(lambda)).^2) + 0.5 * kappa0 * sampler.K / (sampler.K+kappa0) * mean(lambda)^2
    nu2_delta = rand(InverseGamma(a_n, b_n))
    mu_delta = rand(Normal(sampler.K / (sampler.K+kappa0) * mean(lambda), sqrt(nu2_delta / (sampler.K+kappa0))))
    return mu_delta, nu2_delta
end

function log_posterior(sampler::SpatialThetaPostSampler, s, u, beta, sigma2, rho, delta, omega2, eta, mu_delta, nu2_delta, lp_old::Union{Float64, Nothing}=nothing)
    s_flatten = s'[:]
...
function sampling(
    sampler::SpatialThetaPostSampler,
    num_sample::Int;
    burn_in::Int=0,
    thinning::Int=1,
    ...
    delta = isnothing(delta_init) ? zeros(sampler.K) : delta_init
    ...
    delta_samples = zeros(Float64, (num_sample, sampler.K))
