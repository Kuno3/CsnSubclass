...
struct TemporalDeltaPostSampler
    y::Array{Float64,2}
    W::Array{Float64,2}
    ...
    function TemporalDeltaPostSampler(
        y,
        W,
        feature;
        nu2_beta=100.0,
...
function lp_theta(sampler::TemporalDeltaPostSampler, theta, alpha, rhoT, delta, tau2, rhoS)
    b = sqrt(2 / pi)
    ...
    lp_theta += sum(logpdf.(
        Normal.(
            gamma[1] * delta[1] * (alpha[1, :] - b * ones(sampler.K)),
    ...
    for t in 2:sampler.T
        ...
        lp_theta += sum(logpdf.(
            Normal.(
                gamma[t] * delta[t] * (alpha[t, :] - b * ones(sampler.K)),
...
function theta_sampler!(theta, sampler::TemporalDeltaPostSampler, alpha, beta, sigma2, rhoT, delta, tau2, rhoS)
    b = sqrt(2 / pi)
    gamma = 1 ./ sqrt.(1 .- b^2 * delta.^2)
    ...
    cov_theta_pred[1] = Symmetric((1-delta[1]^2) * gamma[1]^2 * Omega)
    mean_theta_pred[1, :] = gamma[1] * delta[1] * Omega_s * (alpha[1, :] .- b)
    ...
    for t in 2:sampler.T
        cov_theta_pred[t] = Symmetric(rhoT^2 * cov_theta_filt[t - 1] + (1-delta[t]^2) * gamma[t]^2 * Omega)
        mean_theta_pred[t, :] = rhoT * mean_theta_filt[t-1, :] + gamma[t] * delta[t] * Omega_s * (alpha[t, :] .- b)
...
function alpha_sampler!(alpha, sampler::TemporalDeltaPostSampler, theta, rhoT, delta, tau2, rhoS)
    b = sqrt(2 / pi)
    gamma = 1 ./ sqrt.(1 .- b^2 * delta.^2)
    inv_Omega_s = (1 / sqrt(tau2)) * sampler.eigen_vector * Diagonal(sqrt.(rhoS * sampler.eigen_values_raw .+ (1-rhoS))) * sampler.eigen_vector'

    tilde_theta = inv_Omega_s * theta[1, :]
    conditional_mu_alpha = delta[1] / gamma[1] * tilde_theta .+ b * delta[1]^2
    alpha[1, :] = conditional_mu_alpha + rand.(Truncated.(Normal(0, sqrt(1-delta[1]^2)), -conditional_mu_alpha, Inf))
    for t in 2:sampler.T
        tilde_theta = inv_Omega_s * (theta[t, :] - rhoT * theta[t-1, :])
        conditional_mu_alpha = delta[t] / gamma[t] * tilde_theta .+ b * delta[t]^2
        alpha[t, :] = conditional_mu_alpha + rand.(Truncated.(Normal(0, sqrt(1-delta[t]^2)), -conditional_mu_alpha, Inf))
    end
end

function beta_sampler(sampler::TemporalDeltaPostSampler, theta, sigma2)
    theta_flatten = theta'[:]
...
function sigma2_sampler(sampler::TemporalDeltaPostSampler, theta, beta)
    theta_flatten = theta'[:]
...
function rhoT_sampler(sampler::TemporalDeltaPostSampler, theta, alpha, delta, tau2, rhoS)
    b = sqrt(2 / pi)
    gamma = 1 ./ sqrt.(1 .- b^2 * delta.^2)
    Omega_s = sqrt(tau2) * sampler.eigen_vector * Diagonal(1.0 ./ sqrt.(rhoS * sampler.eigen_values_raw .+ (1-rhoS))) * sampler.eigen_vector'
    inv_Omega_s = (1 / sqrt(tau2)) * sampler.eigen_vector * Diagonal(sqrt.(rhoS * sampler.eigen_values_raw .+ (1-rhoS))) * sampler.eigen_vector'
    ...
    mu_rhoT = 0
    for t in 2:sampler.T
        mu_theta = gamma[t] * delta[t] * Omega_s * (alpha[t, :] - b * ones(sampler.K))
        theta_scaled = (1 / (gamma[t] * sqrt(1-delta[t]^2))) * inv_Omega_s * theta[t-1, :]
        prec_rhoT += theta_scaled' * theta_scaled
        mu_rhoT += (1 / (gamma[t] * sqrt(1-delta[t]^2))) * (theta[t, :] - mu_theta)' * inv_Omega_s * theta_scaled
    end
...
function delta_sampler!(delta, sampler::TemporalDeltaPostSampler, theta, alpha, rhoT, tau2, rhoS, mu_delta, nu2_delta, delta_prop_scale)
    b = sqrt(2 / pi)
    inv_Omega_s = (1 / sqrt(tau2)) * sampler.eigen_vector * Diagonal(sqrt.(rhoS * sampler.eigen_values_raw .+ (1-rhoS))) * sampler.eigen_vector'
    ...
    for t in 1:sampler.T
        ...
        lp_new = sum(logpdf.(
            Normal.(
                gamma_prop * delta_prop * (alpha[t, :] - b * ones(sampler.K)),
        ...
        lp_old = sum(logpdf.(
            Normal.(
                gamma_curr * delta_curr * (alpha[t, :] - b * ones(sampler.K)),
...
function tau2_sampler(sampler::TemporalDeltaPostSampler, theta, alpha, rhoT, delta, tau2, rhoS, tau2_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
...
function rhoS_sampler(sampler::TemporalDeltaPostSampler, theta, alpha, rhoT, delta, tau2, rhoS, rhoS_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
...
function mu_and_nu2_delta_sampler(sampler::TemporalDeltaPostSampler, delta)
    kappa0 = 0.01
...
function log_posterior(sampler::TemporalDeltaPostSampler, theta, alpha, beta, sigma2, rhoT, delta, tau2, rhoS, mu_delta, nu2_delta, lp_old::Union{Float64, Nothing}=nothing)
    theta_flatten = theta'[:]
...
function sampling(
    sampler::TemporalDeltaPostSampler,
    num_sample::Int;
    burn_in::Int=0,
    thinning::Int=1,
