...
struct SpatialDeltaPostSampler
    y::Array{Float64,2}
    W::Array{Float64,2}
    ...
    function SpatialDeltaPostSampler(
        y,
        W,
        feature;
        nu2_beta=100.0,
...
function lp_theta(sampler::SpatialDeltaPostSampler, theta, alpha, rhoT, delta, tau2, rhoS)
    b = sqrt(2 / pi)
...
function theta_sampler!(theta, sampler::SpatialDeltaPostSampler, alpha, beta, sigma2, rhoT, delta, tau2, rhoS)
    b = sqrt(2 / pi)
    ...
    cov_theta_pred_1 = Symmetric(Omega_s * Gamma * (I - Delta * Delta) * Gamma * Omega_s)
    cov_theta_pred[1] = cov_theta_pred_1
    mean_theta_pred[1, :] = Omega_s * Gamma * Delta * (alpha[1, :] .- b)
    ...
    for t in 2:sampler.T
        ...
        mean_theta_pred[t, :] = rhoT * mean_theta_filt[t-1, :] + Omega_s * Gamma * Delta * (alpha[t, :] .- b)
...
function alpha_sampler!(alpha, sampler::SpatialDeltaPostSampler, theta, rhoT, delta, tau2, rhoS)
    b = sqrt(2 / pi)
    gamma = 1 ./ sqrt.(1 .- b^2 * delta.^2)
    inv_Omega_s = (1 / sqrt(tau2)) * sampler.eigen_vector * Diagonal(sqrt.(rhoS * sampler.eigen_values_raw .+ (1-rhoS))) * sampler.eigen_vector'
...
function beta_sampler(sampler::SpatialDeltaPostSampler, theta, sigma2)
    theta_flatten = theta'[:]
...
function sigma2_sampler(sampler::SpatialDeltaPostSampler, theta, beta)
    theta_flatten = theta'[:]
...
function rhoT_sampler(sampler::SpatialDeltaPostSampler, theta, alpha, delta, tau2, rhoS)
    b = sqrt(2 / pi)
    gamma = 1 ./ sqrt.(1 .- b^2 * delta.^2)
    Delta = Diagonal(delta)
    ...
    mu_rhoT = 0
    for t in 2:sampler.T
        mu_theta = Omega_s * Gamma * Delta * (alpha[t, :] .- b)
        theta_scaled = Diagonal(1 ./ (gamma .* sqrt.(1 .- delta.^2))) * inv_Omega_s * theta[t-1, :]
        prec_rhoT += theta_scaled' * theta_scaled
        mu_rhoT += (theta[t, :] - mu_theta)' * inv_Omega_s * Diagonal(1 ./ (gamma .* sqrt.(1 .- delta.^2))) * theta_scaled
    end
...
function delta_sampler!(delta, sampler::SpatialDeltaPostSampler, theta, alpha, rhoT, tau2, rhoS, mu_delta, nu2_delta, delta_prop_scale)
    b = sqrt(2 / pi)
    inv_Omega_s = (1 / sqrt(tau2)) * sampler.eigen_vector * Diagonal(sqrt.(rhoS * sampler.eigen_values_raw .+ (1-rhoS))) * sampler.eigen_vector'
    dst_prior = Normal(mu_delta, sqrt(nu2_delta))

    tilde_theta = zeros(sampler.T, sampler.K)
    tilde_theta[1, :] = inv_Omega_s * theta[1, :]
    for t in 2:sampler.T
        tilde_theta[t, :] = inv_Omega_s * (theta[t, :] - rhoT * theta[t-1, :])
    end
    ...
    for k in 1:sampler.K
        ...
        lp_new = sum(logpdf.(
            Normal.(
                gamma_prop * delta_prop * (alpha[:, k] .- b),
        ...
        lp_old = sum(logpdf.(
            Normal.(
                gamma_curr * delta_curr * (alpha[:, k] .- b),
...
function tau2_sampler(sampler::SpatialDeltaPostSampler, theta, alpha, rhoT, delta, tau2, rhoS, tau2_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
...
function rhoS_sampler(sampler::SpatialDeltaPostSampler, theta, alpha, rhoT, delta, tau2, rhoS, rhoS_prop_scale, lp_old::Union{Float64, Nothing}=nothing)
...
function mu_and_nu2_delta_sampler(sampler::SpatialDeltaPostSampler, delta)
    kappa0 = 0.01
...
function log_posterior(sampler::SpatialDeltaPostSampler, theta, alpha, beta, sigma2, rhoT, delta, tau2, rhoS, mu_delta, nu2_delta, lp_old::Union{Float64, Nothing}=nothing)
    theta_flatten = theta'[:]
...
function sampling(
    sampler::SpatialDeltaPostSampler,
    num_sample::Int;
    burn_in::Int=0,
    thinning::Int=1,
    ...
    @showprogress for i in 1:(burn_in + num_sample)
        for _ in 1:thinning
            ...
            delta_sampler!(delta, sampler, theta, alpha, rhoT, tau2, rhoS, mu_delta, nu2_delta, delta_prop_scale)
            tau2, lp = tau2_sampler(sampler, theta, alpha, rhoT, delta, tau2, rhoS, tau2_prop_scale)
