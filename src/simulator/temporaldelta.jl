...
struct TemporalDeltaSimulator
    ...
    function TemporalDeltaSimulator(T, W, feature; beta=nothing, sigma2=nothing, rhoT=nothing, delta=nothing, tau2=nothing, rhoS=nothing, seed=1234, theta_init=nothing)
        Random.seed!(seed)
        K = size(W, 1)
        dim = size(feature, 3)
        ...
        delta = isnothing(delta) ? 2 * rand(T) .- 1 : delta
...
function simulate(simulator::TemporalDeltaSimulator)
    b = sqrt(2 / pi)
    ...
    simulator.theta[1, :] = simulator.rhoT * simulator.theta_init + gamma[1] * Omega_s * (sqrt(1-simulator.delta[1]^2) * rand(Normal(0, 1), simulator.K) + simulator.delta[1] * (simulator.alpha[1, :] .- b))
    ...
    for t in 2:simulator.T
        ...
        simulator.theta[t, :] = simulator.rhoT * simulator.theta[t-1, :] + gamma[t] * Omega_s * (sqrt(1-simulator.delta[t]^2) * rand(Normal(0, 1), simulator.K) + simulator.delta[t] * (simulator.alpha[t, :] .- b))
