...
struct TemporalThetaSimulator
    ...
    function TemporalThetaSimulator(T, W, feature; beta=nothing, sigma2=nothing, rho=nothing, delta=nothing, omega2=nothing, eta=nothing, seed=1234, s_init=nothing)
        Random.seed!(seed)
        K = size(W, 1)
        dim = size(feature, 3)
...
function simulate(simulator::TemporalThetaSimulator)
    b = sqrt(2 / pi)
    ...
    simulator.s[1, :] = simulator.rho * simulator.s_init + gamma[1] * Omega_s * (sqrt(1-simulator.delta[1]^2) * rand(Normal(0, 1), simulator.K) + simulator.delta[1] * (simulator.u[1, :] .- b))
    ...
    for t in 2:simulator.T
        ...
        simulator.s[t, :] = simulator.rho * simulator.s[t-1, :] + gamma[t] * Omega_s * (sqrt(1-simulator.delta[t]^2) * rand(Normal(0, 1), simulator.K) + simulator.delta[t] * (simulator.u[t, :] .- b))
