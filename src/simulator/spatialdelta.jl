...
struct SpatialDeltaSimulator
    ...
    function SpatialDeltaSimulator(T, W, feature; beta=nothing, sigma2=nothing, rhoT=nothing, delta=nothing, tau2=nothing, rhoS=nothing, seed=1234, theta_init=nothing)
        Random.seed!(seed)
        K = size(W, 1)
        dim = size(feature, 3)
...
function simulate(simulator::SpatialDeltaSimulator)
    b = sqrt(2 / pi)
    ...
    simulator.theta[1, :] = simulator.rhoT * simulator.theta_init + Omega_s * Gamma * (sqrt(I-Delta.^2) * rand(Normal(0, 1), simulator.K) + Delta * (simulator.alpha[1, :] .- b))
    ...
    for t in 2:simulator.T
        ...
        simulator.theta[t, :] = simulator.rhoT * simulator.theta[t-1, :] + Omega_s * Gamma * (sqrt(I-Delta.^2) * rand(Normal(0, 1), simulator.K) + Delta * (simulator.alpha[t, :] .- b))
