...
struct FixedThetaSimulator
    ...
    function FixedThetaSimulator(T, W, feature; beta=nothing, sigma2=nothing, rho=nothing, delta=nothing, omega2=nothing, eta=nothing, seed=1234, s_init=nothing)
        Random.seed!(seed)
        K = size(W, 1)
        dim = size(feature, 3)
...
function simulate(simulator::FixedThetaSimulator)
    b = sqrt(2 / pi)
    ...
    simulator.s[1, :] = simulator.rho * simulator.s_init + gamma * Omega_s * (sqrt(1-simulator.delta^2) * rand(Normal(0, 1), simulator.K) + simulator.delta * (simulator.u[1, :] .- b))
    ...
    for t in 2:simulator.T
        ...
        simulator.s[t, :] = simulator.rho * simulator.s[t-1, :] + gamma * Omega_s * (sqrt(1-simulator.delta^2) * rand(Normal(0, 1), simulator.K) + simulator.delta * (simulator.u[t, :] .- b))
